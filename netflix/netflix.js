var netflix = {
  today : {},
	init : function() {
    var url = "http://www.smbs.biz/Flash/TodayExRate_flash.jsp?tr_date=2017-07-07";
  ...
  setFlowChart : function(dataSet) {
    ...
    var myChart = new Chart(ctx, {
        type: 'line',
        data: {
            labels: ["5 month ago", "4 month ago", "3 month ago", "2 month ago", "A month ago", "This Month"],
            datasets: [{
  ...
  setInvoiceInfo : function() {
    var schedule = "This is a statement of accounts for Netflix fee from <b>7 " + this.getMonthName(this.today.month) +"</b> to <b>6 "+ this.getMonthName(this.today.month + 1) + "</b>";
    var accounts = "<b>ShinHan Bank(01048077749) / kakaoBank(3333-01-4021489) / KakaoPay & Toss is availalbe</b>"
  ...
  getCurrencyRate : function() {
    ...
    for(var i=0 ; i<6 ; i++) {
      var oldDate = "" + this.today.year + "-" + this.digit(thisMonth, 2) + "-07"
      var requestUrl = "http://data.fixer.io/api/" + oldDate + "?access_key=3c8ef267a2f1e746fafd9ec80b116d80";
    ...
    $.ajax({
        ...
        success : $.proxy(function(res){
            ...
            var amount = "Total amount : <u>$11.99</u>, currency rate : <u>$1 = ₩" + this.currencyRate + "</u> (" + date + ")";
    ...
    Promise.all(promise).then($.proxy(function (values) {
  ...
  calculatePayment : function() {
    ...
    $(".payment").append("₩" + this.numberWithCommas(payment.toFixed(0)) + "($2.99)" );
    $(".total").append("₩" + this.numberWithCommas(payment.toFixed(0)));
    $(".payment-total").append("₩" + this.numberWithCommas(paymentTotal.toFixed(0)) + "($11.99)");
    $(".total-all").append("₩" + this.numberWithCommas(paymentTotal.toFixed(0)));

    var paymentTerm = "The Term of Validity : 6 " + this.getMonthName(this.today.month + 1) + " " + this.today.year;
